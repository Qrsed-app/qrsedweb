...
export class ImagesService {
  ...
  async uploadImage(data: UploadImageData, userId: string): Promise<UploadedImageResponse> {
    ...
    console.log('🖼️ Image upload attempt:', {
      userId,
      roomId,
      noteId,
      fileName: file.name,
    ...
    console.log('🔐 Access check result:', { hasAccess, userId, roomId });
    ...
    console.log('☁️ Uploading to MinIO...', { key: `rooms/${roomId}/notes/${noteId}/${filename}` });
    try {
      ...
      console.log('✅ Database save successful:', imageRecord.id);
